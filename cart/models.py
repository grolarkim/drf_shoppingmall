...
class Cart(models.Model):
    ...
    product_option = models.ForeignKey(
        to=ProductOption, on_delete=models.CASCADE, verbose_name="선택된 상품 옵션"
    )
    count = models.IntegerField(verbose_name="수량", default=1)

    def __str__(self):
        return (
            f"{self.user.username}의 장바구니 {self.product_option.name}, 수량: {self.count}"
        )
...
class PurchaseList(models.Model):
    ...
    payment_method = models.ForeignKey(
        to=PaymentMethod, on_delete=models.CASCADE, verbose_name="결제 방법"
    )
